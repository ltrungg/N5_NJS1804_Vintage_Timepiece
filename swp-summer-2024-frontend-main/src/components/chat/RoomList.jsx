...
export default function RoomList({ list }) {
  return (
    <div className="w-1/4 min-h-[85vh] rounded-l-xl bg-white drop-shadow-xl flex flex-col">
      <p className="w-full px-8 py-4 text-[1.5em] font-bold border-b border-gray-300">CHAT</p>
      {list.length === 0 ? (
        ...
        list.map((item, index) => {
          return <RoomItem key={index} chatRoom={item} />;
        })
      )}
    </div>
  );
